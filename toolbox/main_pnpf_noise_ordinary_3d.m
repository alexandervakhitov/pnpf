...
f= {   @DLT, @upnp_interface,  @upnp_GN_interface, @GPnP_f, @GPnP_f_GN, @RPnP_interface, @(x,y)pnpfmy(x,y,4,1,pnpOpts)};
marker= { 'x', 'o', 'd', '>', 's', '+', '<'};
color= {'r','g','b','m','k','c','b'};
...
for i= 1:length(npts)
    ...
    for j= 1:num
        ...
        if npt >= 6
            ...
            for k= 1:length(method_list)
                 if strcmp(method_list(k).name, 'DLT') || strcmp(method_list(k).name, 'UPnPf') ||...
                         strcmp(method_list(k).name, 'UPnPf+GN') || strcmp(method_list(k).name, 'EPnPfR') 
...
hold('all');
for i = 1:4
    plot(6:15, method_list(i).mean_reproj(3:end),'marker',method_list(i).marker,...
