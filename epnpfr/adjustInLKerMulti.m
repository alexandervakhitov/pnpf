function [xkNs] = adjustInLKerMulti(x0, kerVect, betaNum, L, isFast, haveMex)
%     xkNs = genEigValBasedSolver(x0, kerVect);
%     return;
    errSign = 0;
...
%     if (1)
    ...
    for candInd = 1:size(xks, 2)
        ...
        itmax = 200;
        if (haveMex)
            resFunName = 'LKerResMultiC_mex';
        else
            resFunName = 'LKerResMultiC';
        end
        [ret, popt1, info1, covar] = levmar(resFunName, xStart, imvect, itmax, options, 'blic', ...
